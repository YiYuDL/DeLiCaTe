...
from delicate.clps.PSMolBert_Interface import PSMolbertModel, PSMolbertBatchType
from molbert.tasks.tasks import BaseTask, FinetuneTask
from molbert.utils.featurizer.molfeaturizer import SmilesIndexFeaturizer
from delicate.clps.PSconfig import PSBertConfig

logging.basicConfig(level=logging.INFO, format='%(levelname)s: %(message)s')
logger = logging.getLogger(__name__)


class FinetunePSModel(PSMolbertModel):
    def get_config(self):
    ...
    def test_step(self, batch: PSMolbertBatchType, batch_idx: int) -> Dict[str, Dict[str, torch.Tensor]]:  # type: ignore
        """
    ...
    def test_dataloader(self) -> DataLoader:
        ...
        return MolbertDataLoader(dataset, batch_size=64, num_workers=self.hparams.num_workers)
