...
from delicate.finetune.finetune_KD import FinetuneKD

class finetuneKDApp(BaseMolbertApp):
    @staticmethod
    def get_model(args) -> MolbertModel:
        pprint.pprint(args)
        model = FinetuneKD(args)
        pprint.pprint(model)
        ...
        try:
            d1 = checkpoint['model.bert.embeddings.LayerNorm.bias']
            model.load_state_dict(checkpoint, strict=False)
        
        except:
            aa = 'model.'
            d2 = collections.OrderedDict([((aa+k),v) for k,v in checkpoint.items()])
            model.load_state_dict(d2, strict=False)
        
        if args.freeze_level != 0:
            print('Freezing base model')
            finetuneKDApp.freeze_network(model, args.freeze_level)
