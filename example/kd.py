...
def main():
    ...
    #load bert config
    if args.T_start == 'MolBERT':
        
        Molbert_config = MolBertconfig.from_json_file(args.bert_config_file_T)
        KDMolbert_config = MolBertconfig.from_json_file(args.bert_config_file_S)
   
        model_T = MolBertForDistill(Molbert_config)
        model_S = MolBertForDistill(KDMolbert_config)
        
        weight_T = torch.load(args.tuned_checkpoint_T)
        try:
            d1 = weight_T['bert.embeddings.LayerNorm.bias']
            model_T.load_state_dict(weight_T, strict=False)
        
        except:
            d2 = collections.OrderedDict([(k[6:],v) if k[6:7]
                                          == 'b' else (k[14:], v) for k,v in weight_T.items()])
            model_T.load_state_dict(d2, strict=False)
    
    elif args.T_start == 'PSMolBERT':
        ...
        try:
            d1 = weight_T['psbert.embeddings.LayerNorm.bias']
            model_T.load_state_dict(weight_T, strict=False)
        
        except:
            d2 = collections.OrderedDict([(k[6:],v) if k[6:7]
                                          == 'p' else (k[14:], v) for k,v in weight_T.items()])
            model_T.load_state_dict(d2, strict=False)
